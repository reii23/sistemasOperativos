...
void *calcular_sumatoria(void *arg) {
    ...
    *result = sum;
    ...
    pthread_exit(result); // estado de termino de la hebra, no manda un NULL, envía el resultado
}
...
int main() {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        pthread_join(threads[i], (void **)&result); // como el ptread_exit envía el resultado, se debe recibir el resultado en pthread_join
        total_sum += *result;
        free(result);
    }
